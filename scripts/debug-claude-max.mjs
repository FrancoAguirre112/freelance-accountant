...
const txs = (await c.execute({
  sql: `SELECT id, date, imputed_date, amount FROM transactions
        WHERE service_id = ? AND user_id = ? AND category = 'recurring'
        ORDER BY date ASC;`,
  args: [svc.id, USER_ID],
})).rows;
console.log(`\n${txs.length} recurring txns:`);
for (const t of txs) {
  ...
  console.log(`  id=${t.id} date=${d} imp=${imp ?? "(null)"} amount=${t.amount}`);
}

// Group by imputed||date month
const byMonth = {};
for (const t of txs) {
...
console.log("\nraw sum per month:", byMonth);

// Simulate the walker for today = 2026-05-28
const now = new Date("2026-05-28T12:00:00Z");
const isPayment = svc.type === "payment";
const fee = Math.abs(Number(svc.amount));
console.log(`\nwalker (today=${now.toISOString().slice(0, 10)}, fee=${fee}, type=${svc.type}):`);
for (let offset = -1; offset <= 1; offset++) {
  ...
  if (!covered) {
    ...
    console.log(`    next due = ${dueMonth.toISOString().slice(0, 7)}-${String(svc.billing_day).padStart(2, "0")}`);
    break;
  }
}
